...
export function ProjectDetailsTable({ deliveryUnit, month, financialYear }: ProjectDetailsTableProps) {
  ...
  }, [deliveryUnit, month, financialYear, fetchProjectDetails])
              ...
              <table className="min-w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentData.map((project, index) => {
                    const gmPercentage = Number.parseFloat(project.gm_percentage?.toString() || "0")
                    const isGoodMargin = gmPercentage >= 35

                    return (
                      <tr
                        key={`${project.project_code}-${startIndex + index}`}
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="space-y-1">
                            ...
                            <div className="text-sm">
                              ...
                              <span
                                className={`font-semibold ${gmPercentage >= 0 ? "text-green-600" : "text-red-600"}`}
                              >
            ...
            {data.length > 0 && (
              <div className="bg-gray-50 border-t border-gray-200 px-6 py-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  {totalPages <= 1 && <div className="text-xs text-gray-500">Showing all results on one page</div>}
                </div>
              </div>
            )}
          </>
